...
- `artifacts/` contains per-model metrics (`*_metrics.json`), per-patch test predictions (`*_test_predictions.csv`),    trained model weights (`*.pt`), and the Random Forest results (`rf_results.json`, `rf_feature_importance.csv`, `rf_features.csv`).
